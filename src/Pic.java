...
public class Pic {
	...
	// des images
	/*
	 * static ArrayList<Pic> fromFolder(String path) throws Exception { File folder
	 * = new File(path); File[] files = folder.listFiles(); if (files == null) {
	 * throw new Exception("Folder given does not exit"); }
	 * 
	 * ArrayList<Pic> images = new ArrayList<Pic>(); Integer id = 0; for (File file
	 * : files) { if (!file.isDirectory()) { String name = file.getName(); String[]
	 * bits = name.split("\\."); if (bits.length >= 2) { String extension =
	 * bits[bits.length - 1]; if (extension.matches(Pic.formats)){ id++;
	 * images.add(new Pic(id, name, file, extension)); } } } } return images; }
	 */
